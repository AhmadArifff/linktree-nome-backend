 ...
 */
export async function createProductForCategory(categoryId: string) {
  try {
    ...
    if (error) {
      console.error(
        `Error creating product for category ${categoryId}:`,
        error
      );
      return {
        success: false,
    ...
    console.log(
      `✅ Product created successfully for category ${categoryId}:`,
      data?.[0]?.name
    );
    ...
    console.error(
      `Unexpected error creating product for category ${categoryId}:`,
      error
    );
    return {
      success: false,
...
/**
 * Create products for all categories (called daily at 7:00 AM WIB)
...
export async function createProductsForAllCategories() {
  ...
  console.log(
    `\n🕖 [${new Date().toLocaleString("id-ID", { timeZone: "Asia/Jakarta" })}] Starting daily product creation for all categories...`
  );
  ...
  console.log(
    `\n📊 Daily creation completed: ${successCount} succeeded, ${failureCount} failed`
  );
